...
export function updateZoraContentTokenPricing(
  ...
  const zoraContentResults = new ZoraContentResults('', BigDecimal.zero())
  ...
  if (
    ...
    contentToken = token0
    zoraContentResults.tokenIdentifier = 'Token0'
    creatorToken = token1
  } else if (
    token1.whitelistPools.length == 0 &&
    ...
    contentToken = token1
    zoraContentResults.tokenIdentifier = 'Token1'
    creatorToken = token0
  }
  ...
  if (
    ...
    zoraContentResults.contentDerivedETH = contentToken.derivedETH
  }

  return zoraContentResults
}
